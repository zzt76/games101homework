...
inline Intersection Triangle::getIntersection(Ray ray)
{
    ...
    Vector3f result(dotProduct(s2, e2), dotProduct(s1, s), dotProduct(s2, ray.direction));
    result = (1 / dotProduct(s1, e1)) * result;
    
    t_tmp = result.x;
    u = result.y;
    v = result.z;
    
    if(t_tmp >= 0 && (1 - u - v) >= 0 && u >= 0 && v >= 0){
        inter.happened = true;
        inter.coords = ray(t_tmp); //wi
        inter.normal = normal;
        inter.obj = this;
